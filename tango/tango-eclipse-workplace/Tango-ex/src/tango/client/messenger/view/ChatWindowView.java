...
public class ChatWindowView extends Composite{
		...
		public void onKeyPress(Widget sender, char keyCode, int modifiers){
			if( keyCode == KeyboardListener.KEY_ENTER ){
				...
				if( text.length()>0 ){
					...
					message.setPickupTangoList(Tango_ex.getSavedTangoList());
	...
	/**
	 * @param message
	 * Messageしか引数に与えられなかった場合は
	 * 保有しているcontcactToに対してメッセージを送信する
	 */
	...
	public void addMessage( Contact contact, Message message ){
		HTML messageLabel = new HTML("<b>" + contact.getName() + "</b>: " + message.getMessage()); 
	...
	private void showSavedTangoList(ArrayList<PickupTango> savedTangoList){
		if(savedTangoList.isEmpty()){
			// no action
		}else{
			System.out.println("pickupTangos.size = " + savedTangoList.size());
			for(int i=0;i<savedTangoList.size();i++){
				conversationPanel.add(TangoPanel.tangoPanel(savedTangoList.get(i)));
			}
		}
	}
