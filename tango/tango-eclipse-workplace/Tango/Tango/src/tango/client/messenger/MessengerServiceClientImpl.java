...
public class MessengerServiceClientImpl implements MessengerViewListener{
	...
	protected void handleEvent( Object event ){
		...
		if( event instanceof RefreshShareTangoEvent){
			...
			System.out.print("redundant list=");
			showPickupTangoList(refreshPickupTangoList);

			refreshPickupTangoList = deleteRepeats(refreshPickupTangoList);
	...
	/**
	 * pickupTangoListに格納されている各要素から、重複されている要素を排除したArrayList<PickupTango>を返す.
	 * @param pickupTangoList
	 * @return
	 */
	public ArrayList<PickupTango> deleteRepeats(ArrayList<PickupTango> pickupTangoList){
		for(int i=0; i<pickupTangoList.size()-1; i++){
			for(int j=i+1; j<pickupTangoList.size(); j++){
				if(pickupTangoList.get(i).getName().equals(pickupTangoList.get(j).getName())){
					pickupTangoList.remove(j);
					j--;
				}
			}
		}
