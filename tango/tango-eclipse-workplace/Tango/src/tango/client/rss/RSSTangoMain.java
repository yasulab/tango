...
import tango.client.rss.ui.RSSPanelTable;
...
public class RSSTangoMain extends Composite {
	...
	public RSSTangoMain(boolean flag) {
		...
		// initializeを呼び出す
		if (flag)
			proxy.initialize(PROPERTIES_ADDRESS, callback);

		/*
		 * // getRankingFieldsCollectionの結果に対する処理を定義 callback = new
		 * AsyncCallback<Object>() { public void onFailure(Throwable caught) {
		 * Window.alert("[onFailure] 'caught.getMessage()' is \n" +
		 * caught.getMessage()); }
		 * 
		 * @SuppressWarnings("unchecked") public void onSuccess(Object result) {
		 * if (result != null) rankingCollection = (Set<RankingFields>) result;
		 * else Window.alert("RSSフィードを取得できませんでした。"); } };
		 * 
		 * // getRankingFieldsCollectionを呼び出す if (flag)
